...
tests = Test.label "Terms"
  ...
  xyz' <- Test.term 
    "fun (x:nat) (y:nat) (z:nat) (n:nat) -> add x (mul y z)"
  xnyz' <- Test.term 
    "fun (x:nat) (y:nat) (z:nat) (n:nat) -> add x (mul (add n y) z)"
  
  let xyz = snd . flattenLam . Elea.run $ Float.run xyz'
  let xnyz = snd . flattenLam . Elea.run $ Float.run xnyz'
  
  let test23 = Test.assert (fragmentedUnifierExists xyz xnyz)
