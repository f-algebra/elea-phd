...
data Data
  = Data { _dbTerms :: !(Map String (Polymorphic Term))
...
getName :: Fail.Can m => Term -> Data -> m (String, [Term])
...
stateGetTerm :: (Show Term, Fail.Can m, MonadState Data m) 
  => String -> [Type] -> m Term
...
getTerm' :: (Show Term, Fail.Can m) => String -> [Type] -> Data -> m Term
