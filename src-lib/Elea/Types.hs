...
instance HasType (Term' (Term, Type)) where 
  get (Unr' ty) = ty
  get (Lam' b (_, c)) = Type.Fun (get b) c
  get (Fix' _ b _ xs) = 
    dropArgs (length xs) (get b)
  get (Con' con xs) = 
...
check' (Fix' _ (get -> b_ty) (t, ty) xs) = do
  checkArgs b_ty (map snd xs)
  when (b_ty /= ty) $ do
    t_s <- showM t
    Err.fixBodyBadlyTyped t_s (show b_ty) (show ty)
...
check' (Case' (_, Base cse_ind) falts) =
  ...
  checkAlt' alt_n (Alt' con bs (_, alt_ty))
    | Prelude.get constructorOf con /= cse_ind = Err.incorrectPattern (show con)
    | alt_n /= Prelude.get constructorIndex con = Err.patternsOutOfOrder
